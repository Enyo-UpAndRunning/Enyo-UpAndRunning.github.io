  ...
  for (var i = 0; i < arguments.length; i++) {
    (function (url) {
      ...
      script.onload = function () {
        window.info('Loaded ' + url, 'http://' + window.location.hostname);
        if (url == libraries.coffeescript) window.info('Now you can type CoffeeScript instead of plain old JS!');
      };
...
var exec = document.getElementById('exec'),
    ...
    sandboxframe = injected ? window['JSCONSOLE'] : document.createElement('iframe'),
    sandbox = null,
